...
declare function dke:get-temporal-relevant-notams($db_name as xs:string,$is_id as xs:string,$granularity as xs:integer) as element()
{
  ...
  (: override times for test purposes
  
  let $beginTime := xs:dateTime('2017-06-18T22:04:00.000Z')
  let $endTime := xs:dateTime('2017-06-19T14:10:00.000Z')
  :)
...
(:
This function resolves existing schedules in the given database to active times.
Parameter:
-db_name: database name
-begin: Date and time, where Resolution schould start
-end: Date and time, where Resultion should end
...
declare %updating function dke:resolve-preemptive($db_name as xs:string, $begin as xs:dateTime, $end as xs:dateTime)
{
  ...
  let $messages := $db//*:AIXMBasicMessage
    ...
    (:let $debug := trace($t):)
    return 
    if(exists($t/resolvedPreemptive)) then replace node $t/resolvedPreemptive with $preemptiveNode
    else insert node $preemptiveNode into $t
    
};

(:
This function filter messages with their given schedule.
The schedule has to be resolved to active times

Parameter:
-messages: elements which contains messages
-beginTime: begintime of interested timeperiod
-endTime: endtime of interested timeperiod
...
(:
Checks if there exists an intersection between the given active times and the timeperiod from the input
...
(:
The function handles many timesheets and initiate the recursive call of "resolve-timesheet"
...
(:
Resolves a single timesheet for a given timeperiod and return the active time for this schedule
:)
